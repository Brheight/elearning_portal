...
export default function TestOptionsPage() {
  ...
  const handleFilterTest = async () => {
    ...
    if (selectedYear !== 'all') {
      ...
      try {
        const response = await apiFetch(
          `/assessments/test-options/filter/?year=${selectedYear}&course=${selectedCourse}`
        );
        const result = await response.json();
        if (result.status === 'success') {
    ...
    if (selectedTopic !== 'all') {
      ...
      try {
        const response = await apiFetch(
          `/assessments/test-options/filter/?topic=${selectedTopic}&course=${selectedCourse}`
        );
        const result = await response.json();
        if (result.status === 'success') {
          filteredData = result.data;
...
const handleStartTest = () => {
  ...
  window.location.href = `/assessment/timed-test?${params.toString()}`;
};
